...
Shader "TransparentPet/SlimeLiquid"
{
    Properties
    {
        ...
        // 瞳色（深色瞳孔；高光点固定白色，不随此色变化）
        _EyeColor ("瞳色", Color) = (0.05, 0.07, 0.12, 1)
        ...
        // 视线偏移（xy，范围 ±0.1，C# 传入）——移动瞳孔位置
        _EyeDir ("视线偏移", Vector) = (0, 0, 0, 0)
        // 眨眼 0=睁眼 1=闭眼（闭眼时眼睛沿 y 压扁成线）
        _Blink ("眨眼", Range(0, 1)) = 0
    ...
    SubShader
    {
        ...
        float  _FlowSpeed;
        float4 _EyeDir;             // Vector 属性固定为 float4，使用 .xy
        float  _Blink;
        float  _Squash;
        float  _VelocityW;
        ...
        // ----------------------------------------------------------------
        // eyeMasks —— 单只眼睛的瞳孔/高光遮罩（软边，fwidth 自适应 AA）
        // ----------------------------------------------------------------
        // 数学：
        //   · 瞳孔 = 以 pupilPos 为心的软圆。眨眼不改半径，而是改
        //     "距离度量"：把 y 向距离除以压扁系数 ry（1 → 0.08），
        //     圆就在 y 方向压成一条水平细线（水平宽度不变）——
        //       d' = (d.x, d.y / ry)，e = |d'| / pupilRadius
        //     e 是归一化椭圆距离：e<1 在瞳内，e>1 在瞳外；
        //   · AA 用 fwidth(e)：ry 很小时 ∂e/∂y 巨大 → AA 带在 e 空间
        //     自动变宽 → 换算回屏幕空间始终约 2px，细线不闪烁不锯齿
        //     （这正是步骤1同一套"导数自适应"思想用在任意 SDF 上）；
        //   · 防退化：fwidth 在 2×2 像素 quad 完全无变化时返回 0，
        //     smoothstep 两边界相等会除零，故钳一个最小带宽；
        //   · 高光点固定挂在瞳心左上（光源在左上 → 眼内反光点同侧），
        //     不随 _EyeDir 移动；闭眼时直接淡出（闭上的眼睛没有反光）。
        void eyeMasks(float2 uv1, float2 eyeCenter, float blink, float2 eyeDir,
                      out float pupilMask, out float hlMask)
        {
            const float pupilRadius = 0.12;            // 瞳孔半径（形状坐标单位）
            const float blinkSquash = 0.08;            // 闭眼时 y 向压扁残留
            const float hlRadius = 0.035;              // 高光点半径
            const float2 hlOffset = float2(-0.034, 0.046); // 高光点相对瞳心的左上偏移

            float2 pupilPos = eyeCenter + eyeDir;      // 视线偏移只动瞳孔
            float2 d = uv1 - pupilPos;
            float ry = lerp(1.0, blinkSquash, blink);
            float e = length(float2(d.x, d.y / ry)) / pupilRadius;

            float aa = max(fwidth(e), 1e-4);
            pupilMask = 1.0 - smoothstep(1.0 - aa, 1.0 + aa, e);

            float eh = length(uv1 - (pupilPos + hlOffset)) / hlRadius;
            float aaH = max(fwidth(eh), 1e-4);
            hlMask = (1.0 - smoothstep(1.0 - aaH, 1.0 + aaH, eh)) * (1.0 - blink);
        }
        ...
        Pass
        {
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                // ===== 步骤8：眼睛（最后叠加 —— rim/高光不许盖住眼睛） =====
                // 两只眼睛挂在形状坐标上半部 (±0.34, 0.28)：瞳孔随
                // _EyeDir 移动、闭眼沿 y 压成线。眼睛全在 |uv1|<0.5 的
                // 深处，远离 rim 与 AA 带，后叠加仍轮廓清晰。
                float pupilL, hlL, pupilR, hlR;
                eyeMasks(uv1, float2(-0.34, 0.28), _Blink, _EyeDir.xy, pupilL, hlL);
                eyeMasks(uv1, float2( 0.34, 0.28), _Blink, _EyeDir.xy, pupilR, hlR);
                float pupilMask = max(pupilL, pupilR);
                float hlMask = max(hlL, hlR);
                col = lerp(col, _EyeColor.rgb, pupilMask);        // 深色瞳孔
                col = lerp(col, float3(1.0, 1.0, 1.0), hlMask);   // 白色反光点
                ...
                alpha = max(alpha, pupilMask * 0.55);
