...
| 工程化 | 场景程序化生成（`SceneGenerator`）：不手写场景 YAML，克隆工程一条命令复原全部场景与图标；七个历史版本场景存档（`Assets/Scenes/Versions/`，index 0 = 交付默认）；**94 项 NUnit 测试**（物理/命中/软体/配置/事件总线/表现数学/托盘菜单） |

## 快速开始

**直接体验（Windows）**：运行 `Builds/TransparentPet.exe`（透明桌宠·双窗口形态：液态玻璃主窗口 + 物种副窗口；托盘右键召唤/收回四种史莱姆，左键打开设置窗口）。
