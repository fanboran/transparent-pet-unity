...
| 工程化 | 场景程序化生成（`SceneGenerator`）：不手写场景 YAML，克隆工程一条命令复原全部场景与图标；七个历史版本场景存档（`Assets/Scenes/Versions/`，index 0 = 交付默认）；**156 项 NUnit 测试**（物理/命中/软体/配置/事件总线/表现数学/渲染范围/帧率策略/托盘菜单与 Win32 标志位）；性能与观感改动都带可复现锚定工具（`PerfProbe` 耗时探针，含分段定位 + 配对口径 + `LiquidGlassSnapshot` 定点图 + `UiSnapshot` 面板截图 + `tools/perfcmp.py` 三联对比图） |
