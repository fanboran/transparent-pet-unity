...
- 拖拽抛掷：按住拖动，快甩松手走抛物线；落在任务栏上沿回弹、左右撞墙反弹；接地且速度降下来后**落定悬浮**（静置不再受重力，桌面构图不会自己塌到任务栏上）
...
- 设置窗口：经典白色对话框风，**左列图标页导航（桌宠 / 物理 / 窗口 / 关于）+ 页内滚动**，管理桌宠数量（●×N 增减）、抛射物理、窗口行为（置顶/抓屏隐形/开机自启）；改动秒级热应用到两只窗口
...
| 渲染性能（`Pet/Glass/GlassRenderRect`+`Core/FramePacing`） | 液态玻璃主合成是逐像素重活（每像素贝塞尔 SDF + 法线再 ×2），全屏绘制在核显 2560×1440 上要 **78.6 ms/帧**。改为按包围盒算出绘制矩形，quad / 素材与模糊 RT / 抓屏区域全部收敛到它（shader 端 `_ScreenUvRect` 把 uv 换算回屏幕坐标，画面逐像素不变）；主合成再加**两级剪枝**：远离玻璃的像素按“轮廓外必然全透明”的半径提前退出（该半径由抗锯齿带宽决定、只有 8px，而不是早先误算的阴影尾巴 126px；判定点吸附到 2×2 quad 原点，否则混合 quad 会污染邻居的屏幕空间导数、在轮廓外浮出一圈假阴影），以及逐项 AABB 下界跳过（**下界必须在 AABB 之外才成立**——AABB 内该距离恒为 0，而轮廓内的有符号距离可为负，早期版本因此把"点落在该物品内部"整只误剪，画出体内一条与轮廓平行的缝，见 [待办归档](docs/项目/待办事项.md)）；法线由中心差分改**前向差分**并复用调用方已算出的中心值（每像素 5 次 SDF → 3 次）。同机实测 **4.3 ms/帧（单只，1.9×）**、**11.0 ms/帧（两只融合，2.4×）**（相对上一轮起点；相对最初的整屏绘制累计约 18× / 11×，两只分散在本机核显 2560×1440 下已进 60fps）。**再把 quad 与来源矩形解耦**：主合成只画“轮廓 + 8px”（合成像素数 −71.5%），而模糊/素材/抓屏这些**被采样**的纹理仍留 116px 边距，采样改走 `_BlurRemap`/`_BgRemap` 的屏幕 uv 换算——同进程交替**配对**实测再省 **0.6~0.7 ms/帧**（两只）。另加空闲降帧：拖拽/悬停全速，静置 1.5 秒后按刷新率跳帧到 30fps（核显常驻不再全天候满渲染） |
| 托盘与设置（`Platform/NativeTray`+`UI/SettingsPanel`） | 托盘右键菜单 = Win32 `HMENU`（递归子菜单 `MF_POPUP`）+ **弹出前刷新动态状态**（勾选 `MF_CHECKED` / 单选组 `MFT_RADIOCHECK` 圆点 / 灰显 `MF_GRAYED`——复刻 TrafficMonitor 的 `OnInitMenu` 做法，源码级对照）；菜单里有三个不用开面板的快捷开关（抓屏隐形、开机自启、缩放单选组）。设置窗口 = IMGUI 自绘经典白色对话框，**左列图标+文字页导航 + 右栏内容区 + 页内滚动**，工作副本 + 事件即时生效 + 0.4s 防抖落盘；面板矩形自报悬停 → 面板内可交互、面板外穿透桌面 |
| 工程化 | 场景程序化生成（`SceneGenerator`）：不手写场景 YAML，克隆工程一条命令复原全部场景与图标；七个历史版本场景存档（`Assets/Scenes/Versions/`，index 0 = 交付默认）；**164 项 NUnit 测试**（物理/命中/软体/配置/事件总线/表现数学/渲染范围/帧率策略/托盘菜单与 Win32 标志位/环境切换不变量）；性能与观感改动都带可复现锚定工具（`PerfProbe` 耗时探针，含分段定位 + 配对口径 + `LiquidGlassSnapshot` 定点图 + `UiSnapshot` 面板与融合形态截图 + `tools/perfcmp.py` 三联对比图） |

## 快速开始

**直接体验（Windows）**：从 [GitHub Releases](https://github.com/fanboran/transparent-pet-unity/releases) 下载 `TransparentPet_v0.5.0_win64.zip` 解压后双击 `TransparentPet.exe`（透明桌宠·双窗口形态：液态玻璃主窗口 + 物种副窗口；托盘右键召唤/收回四种史莱姆，左键打开设置窗口）。
...
- **分辨率 / DPI / 任务栏切换**：工作区每帧现取，分辨率变化、DPI 缩放、任务栏移动或自动隐藏、多屏切换之后，静置的史莱姆会被夹回新工作区（不会留在屏幕外或压进任务栏）；融合半径按轮廓全宽计算，与屏幕尺寸无关。**但宠物尺寸仍是固定逻辑像素**，非 100% 缩放屏上会显得偏小；多显示器场景按主屏（抓屏与工作区都取主显示器）
- **"抓屏隐形"的勾选显示的是配置值**，而实际生效值来自场景默认（开机即为开）：在托盘/设置窗口里点第一次看不到变化，点第二次才真正关闭（已登记待办）
...
| 版本 | 形态 |
|---|---|
| **LiquidGlassDesktop**（玻璃线的完成形态） | 真液态玻璃桌面版：抓屏隐形（`WDA_EXCLUDEFROMCAPTURE`）+ 折射真实桌面，**多只同屏**（最多 16 只，smin 融合；融合半径 = 轮廓全宽的 5%，随缩放走、与分辨率无关，贴近才连起来）；可甩可抛 + 落定悬浮 + 呼吸/倾斜/挤压生命感；F11 切换隐形（代价：隐形时录屏/截图中桌宠不可见） |
