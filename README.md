...
| 渲染性能（`Pet/Glass/GlassRenderRect`+`Core/FramePacing`） | 液态玻璃主合成是逐像素重活（每像素 16 槽贝塞尔 SDF + 法线再 ×4），全屏绘制在核显 2560×1440 上要 **78.6 ms/帧**。改为按“轮廓 + 阴影可见半径”算出绘制矩形，quad / 素材与模糊 RT / 抓屏区域全部收敛到它（shader 端 `_ScreenUvRect` 把 uv 换算回屏幕坐标，画面逐像素不变），并给主合成加两级剪枝（远离玻璃整像素提前退出、逐项 AABB 下界跳过）；同机同参数实测 **10.5 ms/帧（单只，7.5×）**、**29.6 ms/帧（两只融合，4.2×）**。另加空闲降帧：拖拽/悬停全速，静置 1.5 秒后按刷新率跳帧到 30fps（核显常驻不再全天候满渲染） |
| 工程化 | 场景程序化生成（`SceneGenerator`）：不手写场景 YAML，克隆工程一条命令复原全部场景与图标；七个历史版本场景存档（`Assets/Scenes/Versions/`，index 0 = 交付默认）；**143 项 NUnit 测试**（物理/命中/软体/配置/事件总线/表现数学/渲染范围/帧率策略/托盘菜单）；性能与观感改动都带可复现锚定工具（`PerfProbe` 耗时探针 + `LiquidGlassSnapshot` 定点图） |
